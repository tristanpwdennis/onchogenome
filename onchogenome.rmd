...
First, I build the metadata from the plate and sample data

```{r buildmetadata}
metadata <-getseqandsamp()
metadata %>% select(sample_id, patient_ID, worm_type) %>% head() %>% kable()
```

**Table 1: A subset of the metadata showing sample, patient ID and sample type**

Then, I calculate the IBS statistics from the genotype distribution, calculated by IBSRelate for all pairs of the samples.
The output of interest are the statistics R1, R0 and KING-Robust Kinship. The statistics are described in more detail by **Waples *et al*, 2018** [1]. (**open access**). 
...
**Table 2: A subset of the pariwise relatedness data showing sample ids, IBS coefficients, type of comparison, fraction of sites involved in calculation, and the known degree of relationship - whether a sample is taken from the same brood or not**

Now, I plot the R0/R1 and R1-Kin from the relatedness data. In [1], the pattern of clustering in these plots clearly delineates the relationship between individuals.
...
**Figure 1: Plot of R0/R1 coefficients for pairs of intrauterine microfilaria. Point colour indicates whether microfilaria of a pair are from the same brood or not**


This is obviously very noisy - we have a lot of individuals that are supposed to be related, who show R0/R1 values of unrelated (see the high R0 values). I applied a filter based on % of the total GL set  used in each pair to calculate the IBS stats and >0.5 (no different to 0.8) worked the best.
...
**Figure 2: Plot of R0/R1 coefficients for pairs of intrauterine microfilaria. Point colour indicates whether microfilaria of a pair are from the same brood or not**
...
[1]: https://onlinelibrary.wiley.com/doi/full/10.1111/mec.14954
